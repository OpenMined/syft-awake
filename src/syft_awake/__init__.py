...
# Create a class to hold the network status display
class _NetworkStatus:
    """Network status display for Jupyter notebooks."""
        ...
        try:
            ...
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #ffffff;">
                <div style="border: 1px solid #e5e7eb; border-radius: 0.375rem; overflow: hidden;">
                    <div style="background: #fee2e2; padding: 1rem 1.25rem; text-align: center;">
                        <h3 style="color: #991b1b; margin: 0;">Network Status Unavailable</h3>
                        <p style="color: #7f1d1d; margin-top: 0.5rem; font-size: 0.875rem;">Unable to fetch network status: {str(e)}</p>
                    </div>
                </div>
            </div>
            """
...
_this_module.network = network

# Add network to __all__
__all__.append("network")

# Clean up namespace completely
_all_names = list(globals().keys())
for _name in _all_names:
    if _name not in __all__ and not _name.startswith('_') and _name not in ['__doc__', '__file__', '__name__', '__package__', '__path__', '__spec__', '__version__', '_repr_html_', 'network']:
        try:
